...
def getData(i, location): 
    ...
    img_path_s2 = f"/content/thesis/Images/s2/{str(i).zfill(4)}.tif"
    img_path_s1 = f"/content/thesis/Images/s1/{str(i).zfill(4)}.tif"
    ...
    writeOutput(imagePatch, img_path_s2, patch_size, coords)
    print(f"s2 images: {i}", imagePatch.shape)

    s1 = ee.ImageCollection('COPERNICUS/S1_GRD').filterBounds(geometry).filter(ee.Filter.eq('instrumentMode','IW')).filter(ee.Filter.listContains('transmitterReceiverPolarisation', 'VH')).filter(ee.Filter.eq('orbitProperties_pass', 'DESCENDING')).filter(ee.Filter.eq('resolution_meters',10))
    s1 = s1.filterDate(beforeStartDate, beforeEndDate)
    s1 = s1.select(['VV', 'VH']).median()
    ndbi_log = s1.expression('10 * log10((VV - VH) / (VV + VH))', {'VV': s1.select('VV'),'VH': s1.select('VH')})
    s1 = s1.addBands(ndbi_log)

    patch, bounds = get_patch(s1, lonlat, patch_size, 10)
    imagePatch = structured_to_unstructured(patch).squeeze()
    coords = np.array(bounds.getInfo().get("coordinates"))[0]
    writeOutput(imagePatch, img_path_s1, patch_size, coords)
    print(f"s1: {i}", imagePatch.shape)
